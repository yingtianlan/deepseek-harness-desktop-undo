...
// The bare package name is pre-registered by the client module system, so the
// dialog module needs its own suffix — reusing the package id throws
// "duplicate factory registration" and the dialog silently never applies.
  try {
    globalThis.__ModuleLoader__.load({ id: 'dsh-tauri-turnrewind/dialog', factory: () => {
      const module = { exports: {} }
      const exports = module.exports
  ...
  // A duplicate registration means the bundle executed twice (HMR without
  // invalidate); the first registration is still alive, so just surface it.
    console.error('[turnrewind] dialog module registration failed:', error)
  }
}
