 ...
 */
import { MENU_VIEWPORT_MARGIN } from '../constants'
...
export function createMenuItem(options: MenuItemOptions): HTMLButtonElement {
  ...
  button.type = 'button'
  button.className = options.danger ? `${'dshp-menu__item'} ${'dshp-menu__item--danger'}` : 'dshp-menu__item'
  ...
  if (options.shortcut) {
    ...
    hint.className = 'dshp-menu__shortcut'
...
export function createSeparator(): HTMLDivElement {
  ...
  node.className = 'dshp-menu__separator'
