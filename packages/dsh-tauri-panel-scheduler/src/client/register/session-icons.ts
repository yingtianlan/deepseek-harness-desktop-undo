 ...
 *
 * 归属判定：schedulerStore.runs 携带的 sessionId 集合（执行记录与面板轮询刷新
...
export function installSessionIcons(): () => void {
   ...
   * so read the private React Fiber key (only read, never move React-managed nodes).
   */
  ...
  function scan(): void {
    ...
    for (const run of schedulerStore.getSnapshot().runs) {
      if (run.sessionId)
  ...
  controller.add(schedulerStore.subscribe(scan))
